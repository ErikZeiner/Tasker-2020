...
class DateActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        taskViewModel.tasks.observe(this, Observer {tasks ->
            tasks?.let { dateAdapter.setData(it) }
        })

    }
    fun getTasksWhereDate(date:String):List<Task>{
        val projectViewModel = ViewModelProvider(this@DateActivity).get(ProjectViewModel::class.java)
        val taskViewModel = ViewModelProvider(this@DateActivity).get(TaskViewModel::class.java)
        val taskAdapter = TasksAdapter(this@DateActivity)

        var list: List<Task> = mutableListOf()

        taskViewModel.tasks.observe(this@DateActivity, Observer {tasks ->
            tasks?.let { taskAdapter.setTask(it) }
             list = taskAdapter.getDateTasks(date).toMutableList()
        })
        return list
